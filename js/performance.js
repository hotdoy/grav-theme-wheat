(function () {
	...
	function getCo2() {
		...
		E = (B / 1e9) * 0.81 + (B / 1e9) * 0.81 * 0.25 * 0.02; // Energy
	...
	function logPerformance(logToconsole, logTodom) {
		if (logToconsole) {
			console.log(
				`⚡This site became interactive in ${tti}s and complete in ${ttc}s`
			);
			console.log(
				`🌲The initial load transfered around ${(B / 1024).toPrecision(
					5
				)} KB for an estimate of ${C.toPrecision(2)}g of CO2.`
			);
		}
		if (logTodom) {
